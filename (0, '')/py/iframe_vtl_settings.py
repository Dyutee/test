...
try:
	...
	if(form.getvalue("submit_add_target")):
		...
		get_select_opt = form.getvalue("check_portal_opt")
		get_sel_port = form.getvalue("portal_name")

		if (get_select_opt == "select_all_iface"):
			ips = "*"
		else:
			if get_sel_port != None:
				ips = get_sel_port
			else:
				ips = ''
	...
	print """
		...
		<div style="margin:0 0 0px 0;" class="topinputwrap-heading"><a class="demo" href ="#"><img src ="../images/help_icon1.png" style="width:13px;"><span class="tooltip" >
                 <table border="0">
        <tr>
        <td class="text_css">On this page you can create new VTL libraries, as well as view, disable or delete existing ones. You can also add iSCSI, FC and SRP targets to those libraries, and view session details for the enabled libraries.</td>
</tr>
        </table>"""
	if(check_ha == True):
		print"""
	</span></a> VTL Information ("""+show_tn+""")
                <span style="float:right; margin:0;"><a onclick="return onclick_loader();" class="resource_links" href="https://"""+other_node_ip+"""/fs4/py/iframe_vtl_settings.py">"""+show_on+"""</a></span>
                        </div>"""
	else:
		print """ </span></a><p class = "gap_text">VTL Information</p> </div>"""
	print"""
		...
		<tr>
		<td style="color:#666666;">Select VTL Disk</td>
		<td>
		...
		if get_all_vtls['id']== 0:
			if len(get_all_vtls['vtl_libs']) != 0:
				for lib in get_all_vtls['vtl_libs']:
					...
					if(lib['lib_status'] == 'online'):
						print """ <button type='submit' name='on_but' value='"""+str(lib['lib_id'])+"""' style='background-color:#FFF; border:none; cursor:pointer;' onclick='return make_offline("""+str(lib['lib_id'])+""");' title='Click to make Offline.'><img src = '../images/active.png' alt='online' id='sync-static-make-off"""+str(lib['lib_id'])+"""' /><img  height="25px" width=""25px id='sync-loading-make-off"""+str(lib['lib_id'])+"""' style="display:none;" src="../images/sync-loading.GIF" alt="" /></botton>"""
					else:
						print """ <button type='submit' name='off_but' value='"""+str(lib['lib_id'])+"""' style='background-color:#FFF; border:none; cursor:pointer;' onclick='return make_online("""+str(lib['lib_id'])+""");' title='Click to make Online.'><img src = '../images/inactive.png' alt='offline' id='sync-static-make-on"""+str(lib['lib_id'])+"""' /> <img  height="25px" width=""25px id='sync-loading-make-on"""+str(lib['lib_id'])+"""' style="display:none;" src="../images/sync-loading.GIF" alt="" /></button>"""
	...
	<table width="100%">
	<tr>
	<td style="color:#666666;">Select VTL Library</td>
	<td>
	<div class="styled-select2">
	...
	if get_all_vtls['id']== 0:
		if len(get_all_vtls['vtl_libs']) != 0:
			for lib in get_all_vtls['vtl_libs']:
				...
				print """<option """+disabled_q+""" value='"""+str(lib['lib_name']).replace(' ','')+'VTL'+str(post_lib_name)+':'+str(lib['lib_id'])+"""'"""
				if(g_library_name != ''):
	...
	<tr>
	<td style="color:#666666;">Target Name</td>
	<td>"""
	if(g_library_name != ''):
	...
	<tr>
	<td style="color:#666666;">Enter Initiator Name</td>
	<td>
	...
	<tr>
	<td width = "200" class = "table_heading" height = "40px" valign = "top" bgcolor = "#f5f5f5" style="color:#666666;">
	Check the Portal(s) to Add
	</td>

	<td>
	<input type="radio" name="check_portal_opt" value="select_all_iface" onclick="return show_on_select('show_all');" checked />Select All
	<input type="radio" name="check_portal_opt" value="select_one_iface" onclick="return show_on_select('show_one');"/>Select a Device<br/><br/>
	<div class="styled-select2" id="show_on_sel" style="display:none;">
	<select name="portal_name">"""
	for x in get_all_iface['all_conf']:
		if x['address'] != '':
			print """<option value='"""+x['address']+"""'>"""+x['iface']+""" """+x['address']+"""</option>"""
	print """</select>
	</div>
