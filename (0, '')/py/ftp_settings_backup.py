#!/usr/bin/python
import cgitb, sys, header, share_details, commands, common_methods, sys, cgi, os
cgitb.enable()
...
try:
        ...
        #all_ftp_shares = ftp_auth.get_all_shares()
        #print all_ftp_shares
        #delete_all = ftp_auth.delete_all_shares()
        ...
        ftp_write_ip = ''
        sharename= ''
        #ftp_all_mount = commons.mount_ftp_all();
        ...
        if(header.form.getvalue("ftp_button")):
                ...
                #print "AUTH FILE : "+str(check_auth_file_existance)
                #print "<br/>"
                #print "ANON FILE : "+str(check_anon_file_existance)
                #print "<br/>"
        ...
        ads_domain_for_ftp = commands.getoutput('wbinfo --own-domain');
	...
	ftpgroupslength = ''
        ...
        if (all_users_list['id'] == 0):
                if (share_details.connstatus == 'Join is OK'):
                        ...
                        if (checkusersfile[0] == 0):
                                ftp_all_users_array = open('/tmp/adsusersfile', 'r');
                                ftpuserslength      = common_methods.get_users_count();

                else:
                        ftp_all_users_array  = all_users_list['users'];
                        ftpuserslength       = len(ftp_all_users_array);
        ...
        if (all_groups_list['id'] == 0):
                if (share_details.connstatus == 'Join is OK'):
                        ...
                        if (checkgroupsfile[0] == 0):
                                ftp_all_groups_array = open('/tmp/adsgroupsfile', 'r');
                                ftpgroupslength      = common_methods.get_groups_count();

                else:
                        ftp_all_groups_array = all_groups_list['groups'];
                        ftpgroupslength      = len(ftp_all_groups_array);
        ...
        # get the authorized users from the script '/var/nasexe/get_ftp_auth_users user "share_name"'
        get_ftp_auth_users_command = 'sudo /var/nasexe/get_ftp_auth_users user "\/' + share_1 + '\>"';
        get_ftp_auth_users = commands.getoutput(get_ftp_auth_users_command);

        # replace the text 'AllowUser' with '' in the line 'AllowUser....' and trim the output
        get_ftp_auth_users = get_ftp_auth_users.replace('AllowUser', '');
        get_ftp_auth_users = get_ftp_auth_users.strip();

        # get the authorized groups from the script '/var/nasexe/get_ftp_auth_users group "share_name"'
        get_ftp_auth_groups_command = 'sudo /var/nasexe/get_ftp_auth_users group \/' + share_1 + '\>';
        get_ftp_auth_groups = commands.getoutput(get_ftp_auth_groups_command);

        # replace the text 'AllowGroup' with '' in the line 'AllowGroup....' and trim the output
        get_ftp_auth_groups = get_ftp_auth_groups.replace('AllowGroup', '');
        get_ftp_auth_groups = get_ftp_auth_groups.strip();
        ...
        check_shares1 = commands.getoutput('sudo grep "/%s>" /var/nasconf/ftp_anonymous.conf' % sharename);
        check_shares2 = commands.getoutput('sudo grep "/%s>" /var/nasconf/ftp_users.conf' % sharename);
	...
	print """
		...
		<div class="topinputwrap-heading">FTP Settings for '"""+get_share+"""'</div>
		  <div class="topinputwrap">



 <form name = 'set_ftp_params' method = 'POST'>
                                <table width = "685" border = "0" cellspacing = "0" cellpadding = "0" name = 'disp_tables' id = 'id_ftp_settings' style = 'display:block;'>
                                        <tr>
                                ...
                                <div width = '100%' id = 'id_ftp_parameters' style = 'display:  block;'>
                                <table width = '100%'>
                                        <tr>
                                                <td colspan = '2'>
                                                        <input type = 'checkbox' name = 'ftp_read_only' onclick = 'return set_ftp_parameters();'  """ + readonly_disable + ' ' + readonly_checked + """ """ + share_details.alldisabled + """><B>Read only</B><BR><BR>"""

        print """<input type = 'radio' name = 'choose_ftp_options' value = 'anonymous' id ="anon" onclick = 'return set_ftp_parameters();' ><B>Anonymous mode</B><BR>
                                                        <input type = 'radio' name = 'choose_ftp_options' id ="auth" value = 'authenticated' onclick = 'return set_ftp_parameters();' ><B>Authenticated user</B><BR><BR>
                                                </td>
                                        </tr>
                                </table>
                                </div>
                                ...
                                <table style = 'font-weight: bold;'>
                                        <tr>
                                                ...
                                                <td>
                                                        <input class = 'input' name = 'ftp_access_ip' id = 'id_access_ip' value = '""" + ftpaccess_ip + """' size = '55'""" + share_details.alldisabled + """>
                                                </td>
                                        </tr>
                                        <tr>
                                                <td>
                                                ...
                                                <td>
                                                        <input class = 'input' name = 'ftp_write_ip' id = 'id_write_ip' value = '""" + ftpwrite_ip + """' style = 'background:  """ + write_ip_background + """;' size = '55'  """ + write_ip_disabled + """ """ + share_details.alldisabled + """>
                                                </td>
                                        </tr>
                                </table>
                                </div>"""
        ...
        if (len(ftp_all_users_array) > 1000):
                ...
                print """<select class = 'input' style = 'width: 200px; height: 150px;' id = 'ftp_available' name = 'avail_users' multiple onclick = "return ftp_move_users(this.form.ftp_available, this.form.ftp_granted, '1');"  """ + share_details.alldisabled + """>"""
        ...
        print """                                                                       </td>
                                                                                <td>
                                                                                        <select class = 'input' style = 'width: 200px; height: 150px;' id = 'ftp_granted' name = 'grant_users[]' multiple onclick = "return ftp_move_users(this.form.ftp_granted, this.form.ftp_available, '%s');" %s >""" % (count_usr, share_details.alldisabled);
        ...
        if (len(ftp_all_groups_array) > 1000):
                print """<input id = 'ftp_available_groups' name = 'ftp_ads_group_text' type="text" class = 'input' value = '' oninput = 'generate_user_list(document.getElementById("ftp_available_groups"), document.getElementById("ftp_g_autosuggest"), "%s", "groups", "set_ftp_params");' onclick = 'document.getElementById("ftp_g_autosuggest").style.display = "none";'> <input class = 'input1' type = 'button' name = '' value = '>' onclick = "ftp_move_group_to_dropdown(this.form.ftp_available_groups, this.form.ftp_granted_groups, '1');" %s >""" % (ads_groups_only_string, share_details.alldisabled);
                print """<select class = 'input' style = 'width: 250px; height: 350px; display: none;' id = 'ftp_g_autosuggest' name = 'avail_groups' multiple onclick = 'return set_user(this.form.ftp_available_groups, this.form.ftp_g_autosuggest, this.form.ftp_g_autosuggest.value);'   """ + share_details.alldisabled + """>"""
                print ftp_groups_list
                print """</select>"""

        else:
                print """<select class = 'input' style = 'width: 200px; height: 150px;' id = 'ftp_available_groups' name = 'avail_groups' multiple onclick = "return ftp_move_groups(this.form.ftp_available_groups, this.form.ftp_granted_groups, '1');" """ + share_details.alldisabled + """>"""
                groups_only = '';
        ...
        print """                                       </td>
                                                <td>
                                                        <select class = 'input' style = 'width: 200px; height: 150px;' id = 'ftp_granted_groups' name = 'grant_groups[]' multiple onclick = "return ftp_move_groups(this.form.ftp_granted_groups, this.form.ftp_available_groups, '%s');" %s>""" % (count_grp, share_details.alldisabled)
        ...
        print """                                       </td>
                                                <td align = 'right'>
<button class="button_example" type="submit" name = 'local_action_but'  id = 'local_action_but' value = 'Apply'  onclick = 'validate_local_auth();'>Apply</button>
                                                         <!--<input class = 'input1' type = 'button' name = 'ftp_button' value = 'Apply' onclick = 'return validate_ftp_parameters();' >-->
