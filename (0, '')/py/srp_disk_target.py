...
if(header.form.getvalue('assign_disk_nm')):
	...
	print "<script>location.href = 'main.py?page=disk_srp#tabs-1';</script>"
...
if(header.form.getvalue('add_disk_action')):
	...
	if((target_assign_to_disk) == True):
		...
		print "</div>"
		print "<script>location.href = 'main.py?page=disk_srp#tabs-1';</script>"
		#print "<script>location.href = 'main.py?page=srp&act=assign_target_done';</script>"
	else:
...
if(header.form.getvalue('delete_target_name')):
	...
	print "<script>location.href = 'main.py?page=disk_srp#tabs-2';</script>"
...
if (str(check_srp).find("'1'") > 0):
	...
	print"""<table width = "685" border = "0" cellspacing = "0" cellpadding = "0">
			...
			<select name = 'assign_disk_nm' onchange='this.form.submit()' style="width: 347px;">
			<option value='assign_tar'>Select Target</option>"""

	if (srp_target != [{}]):
		for assign_disk in srp_target:
			#print x
	...
	if (srp_target != [{}]):
		for delete_assign_disk in srp_target:
			#print x
	...
	<button class = 'buttonClass' type="submit" name = 'remove_disk' value = 'Remove selected disk' onclick ='return validate_del_srp_assign_target();'>Delete</button>
