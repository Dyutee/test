...
query = 'select * from disk_usage where disk_name="'+int_type+'" and timestamp between '+s_from+' and '+s_to+';'
...
for x in execute["output"]:
        ...
        data_val2 += '{x:'+str(i)+', y:'+x["kb_read"]+', label:'+'"'+str(date_str)+'"'+'},'
        data_val3 += '{x:'+str(i)+', y:'+x["kb_write"]+', label:'+'"'+str(date_str)+'"'+'},'
	i += 1
#print data_val2[:-1]
...
<body>
<div id="chartContainer" style="height: 400px; width: 99%; border:#585858 1px solid; -webkit-box-shadow: 7px 10px 5px 0px rgba(0,0,0,0.75);
