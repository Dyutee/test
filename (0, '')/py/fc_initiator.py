...
show_target_ini = ''
###############Add Initiator###########################
if(header.form.getvalue('fc_ini_but')):
	...
	#print initiator_status
		if(initiator_status == True):
			...
			print "</div>"
			print "<script>location.href = 'main.py?page=fc_ini#tabs-1';</script>"
...
if (check_fc !=[]):
	...
	<div class="insidepage-heading">Fc >> <span class="content">Fc Initiator</span></div>
	<!--tab srt-->
	...
	if (fc_target != [{}]):
		for initiator_target in fc_target:
			#print x
	...
	if (fc_target != [{}]):
		for initiator_del_target in fc_target:
			#print x
	...
	<button class = 'buttonClass' type="submit" name = 'delete_initiator' value = 'Remove initiator' onclick ='return validate_fc_remove_ini();'>Remove Initiator</button>
