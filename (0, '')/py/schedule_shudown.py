...
#--------------Schedule Shutdown------------------------------
#---------------First Step------------------------------------ 
if(form.getvalue("sched_shutdown")):
...
#---------------------------------------------End------------------------------------------------	
	...
	sched_val = str(per_min_val)+' '+str(per_hour_val)+' '+str(per_day_month_val)+' '+str(per_month_val)+' '+str(per_day_val)
	if "/*" in str(sched_val):
		sched_val= sched_val.replace("/*", '')
	#print 'OUTPUT:'+str(sched_val)

	
	sched_shutdown_status = schedule_shutdown.create_schecule_shutdown_script(sched_val, debug=False)
	#print sched_shutdown_status
	if(sched_shutdown_status == True):
        	print"""<div id = 'id_trace'>"""
                print "Successfully Schedule the Shutdown!"
                print "</div>"
       	else:
...
print """
 ...
 <form name='form1' method='post' action=''>
		<h1 style="color:#696666;"> Schedule ShutDown </h1>
		...
		<tr>
			<td style="border:#D1D1D1 1px solid; padding:5px;"><input type = "checkbox" name = "min_check" """+minute_chk_val+""" id="chk" style="margin-left: 44%;" onclick = 'return sched_shutdown_hide();'></td>
			<td style="border:#D1D1D1 1px solid; padding:5px;"><input type = "checkbox" name = "hour_check" """+hour_chk_val+""" id="chk1" style="margin-left: 44%;" onclick = 'return sched_shutdown_hide1();'></td>
			<td style="border:#D1D1D1 1px solid; padding:5px;"><input type = "checkbox" name = "day_month_check" id="chk2" """+day_month_chk_val+""" style="margin-left: 44%;" onclick = 'return sched_shutdown_hide2();'></td>
			<td style="border:#D1D1D1 1px solid; padding:5px;"><input type = "checkbox" name = "month_check" id="chk3" """+month_chk_val+""" style="margin-left: 44%;" onclick = 'return sched_shutdown_hide3();'></td>

			<td style="border:#D1D1D1 1px solid; padding:5px;"><input type = "checkbox" name = "day_check" id="chk4" """+day_chk_val+""" style="margin-left: 44%;" onclick = 'return sched_shutdown_hide4();'></td>
		</tr>
		<tr>
...
<div style="height:29px; font-size:small;width:68px;float:right;margin-right:7%;margin-top:2%;">
         ...
         <button class="buttonClass" type="submit" style="height: 29px; font-size: small; width: 68px;" name = 'sched_shutdown' value = 'sched_val' onclick = 'return validate_snapshot_schedule();'>Schedule</button>
</div>
