...
try:
	...
	#------------------End------------------------------
	nw_path = []
	if(header.form.getvalue('selected_file')):
	...
	if(header.form.getvalue('set_acl')):
		select_domain = header.form.getvalue('domainslist')
	...
	if(header.form.getvalue("re_assign_ownership")):
                share_path = header.form.getvalue('acl_path')
		set_path = share_path
		share_path_add = '/storage/'+share_path
		#print share_path_add
                set_owner  = header.form.getvalue('assd_user')
		#print set_owner
                set_group  = header.form.getvalue('assd_group')
		#print set_group
                recursive  = header.form.getvalue('inherit_ownership')
		#print recursive
		if (recursive == 'on'):
                        recursive = 'YES'
			#print recursive
		...
		if(ownership_status == True):
			...
			print "</div>"
		print "<script>location.href = 'main.py?page=acl#subtabs-4';</script>"

	#---------------------------End---------------------------------------
	#---------------------------Reset OwnerShip---------------------------------
	if(header.form.getvalue('reset_ownership')):
		share_path_o = header.form.getvalue('acl_path')
	...
	if(header.form.getvalue("reset_acl")):
		...
		print "<script>location.href = 'main.py?page=acl#subtabs-6';</script>"
	...
	for reset_info in lis_info:
		...
		if(header.form.getvalue(reset_info+"reset_single")):
			...
			print "<script>location.href = 'main.py?page=acl#subtabs-5';</script>"
			#print """<script>location.href = 'main.py?page=acl&"""+querystring+"""#subtabs-5';</script>"""
	...
	if(header.form.getvalue('set_p')):
		...
		#set_path = header.form.getvalue('share_hid_path')
		#print set_path
	...
	if(header.form.getvalue("action")):
		if(header.form.getvalue("action") == 'own_value'):
                	print"""<div id = 'id_trace'>"""
	...
	print"""<table width="100%">
		<tr>
		<td style="color:#585858; font-weight:600;">Folder:</td>
		<td colspan = '4'>
		...
		<input type="hidden" name="share_name" value='"""+get_share+"""' />
		  ...
		  <tr>
		  <td style="color:#585858; font-weight:600;">Share Path:</td>
	...
	print"""<div style="float:right;margin-top:-3%;">
		<button class="button_example" type="submit" name = 'set_p'  id = 'id_create_but'  value = 'set_p'>Set Path</button></div>
		  </div>"""
	print"""
	</form>
	...
	print"""
		 <tr>
		  <td style="color:#585858; font-weight:600;"><b>Share Path</b>:</td>
			<!--access_control_form.selected_file-->
		 <input type="hidden" name="share_name" value='"""+get_share+"""' />
		<input type="hidden" name="share_hid_path" value='"""+set_path+"""' />
		  <td colspan = '4'>"""
	...
	print"""</td>
		...
		<tr>
			<td>Choose a domain:<div class="styled-select2" style="width:184px;"><select name = 'domainslist'>
	...
	print """</select></div></td></tr>
		<tr>
		<td style="color:#585858; font-weight:600;"><B>Available:</B><BR>
	...
	print """<select name ="avail[]" multiple class="user" id ="avail_id" onkeydown = 'return get_key();' style ='width:250px;height:300px; display: """ + avail_users_style + """;'>"""
	...
	print """	<td style="color:#585858; font-weight:600;"><B>Authorised:</B><BR>
	...
	print """ 	<td>
		<button class="button_example" type="submit" name = 'set' id = "set_id" value = 'Acl' onclick = 'return validate_set_acl();'>Set</button></td>
	...
	<form name = "perm_form" method = "post" action='main.py?page=acl&"""+querystring+"""#subtabs-3' >
	...
	<table style="width:84%;margin:20px 0 0 0;">
		...
		<input type="hidden" name="share_name" value='"""+get_share+"""' />
		<input type="hidden" name="share_hid_path" value='"""+set_path+"""' />
		<td style="color:#585858; font-weight:600;">Share Path:"""
	if(set_path !=''):
		print """ 
	...
	print"""
		...
		<tr>
			<th style = "color:DarkOliveGreen;font-family:menu;font-size:14px;"><b style="margin-left:16%;">Read</b></th>
		...
		<tr>
			<th align="left" style="padding:0 0 0 20px; color:#585858;">Owner</th>
	...
	print"""></th>
		...
		<tr>
			<th align="left" style="padding:0 0 0 20px; color:#585858;">Group</th>
	...
	print"""></th>
		...
		<tr>
			<th align="left" style="padding:0 0 0 20px;color:#585858;">Other</th>
	...
	print"""></th>
		...
		<div style="float:right;margin-top:-3%;">
		<button class="button_example" type = 'button'  name="change_perm_but" onclick="popup('proppopUpDiv7')" value="change_perm_but" >Change Permission</button></div>
...
<br/>
        ...
        <button class="button_example" type="button" name = 'reset_ownership'  id = 'reset_ownership' onclick="popup('proppopUpDiv8')" value = 'ResetOwner'  style="float:right; margin:20px 10px 10px 0;" >Reset Ownership</button>
	
<button class="button_example" type="button" name = 're_assign_ownership'  id = 're_assign_ownership' onclick="popup('proppopUpDiv9')" value = 'Apply'  style="float:right; margin:20px 10px 10px 0;" >Set Ownership</button>
</div>
	...
	<form name="acl_info" method="post" action=''>
        ...
        <div style="text-align:center; height:70px; margin:20px 0 20px 0;">
        Are you sure you want to Update Acl?<br/><br/>
	...
	if(grant_user == None):
		...
		for users_list in lis_info:
			...
			print"""<td style="float:right;width:73%;margin-top:-2%;"><input type="checkbox" name='"""+users_list+"""o_read'"""
			if(chk_perm != {}):
			...
			print"""<td style="float:right;width:56.5%;margin-top:-2%;"><input type="checkbox" name='"""+users_list+"""o_write'"""
			if(chk_perm != {}):
			...
			print """></td>"""
			print"""<td style="float:right;margin-right:36%;margin-top:-2%;"><input type="checkbox" name='"""+users_list+"""o_execute'"""
			if(chk_perm != {}):
			...
			print"""<td style="float:right;margin-right:19%;margin-top:-3%;"><button type="image" value = "reset" name='"""+users_list+"""reset_single' style="background: #fff; border: 0px;cursor:pointer;" ><img src="../images/snap_del_but_small.jpg"></button></td> """
	...
	print"""<td style="color:#585858; font-weight:600;"><div style="margin-top: 3%;"><b>Recursive</b>:<input type="checkbox" name='o_recursive' """+recr_chk+""" /></div></td>"""
	...
	#if(get_all_users['users'] != []):
	print"""<div style="float:right;margin-top:-3%;">
		<button class="button_example" type="button" name = 'acl_user_del' onclick="popup('proppopUpDiv10')" value = 'info'>Reassign</button></div>"""
	#else:
	...
	print"""
		...
		<tr>
		<td style="color:#585858; font-weight:600;">Recursive:</td>"""
	if(reset_chk == 'on'):
	...
	#if(get_all_users['users'] != []):
	print"""<div style="float:right;margin-top:-3%;">
		<button class="button_example" type="button" name = 'reset_acl' onclick="popup('proppopUpDiv11')" value = 'Remove' >Remove</button></div>"""
	#else:
