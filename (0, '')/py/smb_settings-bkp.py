...
try:
	...
	log_path_status = smb_logpath.is_set()
		...
		enable_audit_mess = "[<font style='color:#DF0101;'>SMB Log Path is not enabled!</font> <a href='main.py?page=logs#tabs-4' style='text-decoration:underline;'>Enable Now</a>]"
	...
	if (test_for_smb == '' and connstatus == 'Join is OK'):
		...
		<div style="padding:10px; background-color:#e9e5e5; border-bottom:#D1D1D1 1px solid; font-weight:bold;">SMB Settings for '"""+get_share+"""' <a href = 'main.py?page=cs'><img style="float:right; padding:0px;" title = 'Back to shares' src = '../images/gobacktoshares.png' /></a></div>
	...
	<input type="hidden" name="get_ha_nodename" value='"""+get_ha_nodename+"""' />
		...
		print """
											...
											<td><B>Authorized Groups:</B> <BR><div style="float: right;" align = 'right'>
		...
		print """</select>
				...
				<input id = 'id_select_adv' type = 'checkbox' name = 'aud_recycle' onclick = 'return show_hide(document.getElementById("id_select_adv").checked, document.getElementById("id_adv_feature"));' style = 'margin-left: 4%;' """ + audrecycle_checked + """> <b style="color:#7F7979;">Enable Auditing/Recycling</b><br/><br/>
		...
		<input id = 'id_enable_recycle' type = 'checkbox' name = 'enable_recycle' onclick = 'return show_hide(document.getElementById("id_enable_recycle").checked, document.getElementById("id_recycle_options"));' """ + recycle_checked + """>&nbsp; <B>Enable Recycling</B><br/><br/>
		...
		if (use_smb == 'on'):
			print """
			<button class="button_example" type="submit" name = 'removeconf'  value = 'removeconf'  style="float:right;">Remove Configuration</button>
			<button class="button_example" type = "submit"  name="reconf" value="reconf" style="float:right;" >Update Configuration</button>"""

		else:
			print """<button class="button_example" type="submit"  name='conf' value='conf' style="float:right;" >Configure</button>"""
