...
query = 'select * from temperature where timestamp between '+s_from+' and '+s_to+';'
...
for x in execute["output"]:
        ...
        data_val2 += '{x:'+str(i)+', y:'+x["cpu1"]+', label:'+'"'+str(date_str)+'"'+'},'
        data_val3 += '{x:'+str(i)+', y:'+x["cpu2"]+', label:'+'"'+str(date_str)+'"'+'},'
	i += 1
#print data_val2[:-1]
