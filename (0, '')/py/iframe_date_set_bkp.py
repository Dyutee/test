...
try:
	...
	#if (check_ntp == ''):
	#       ntpdisabled = 'disabled';
	#       message = '<font color = "darkred" size = "2">(Could not locate NTP configuration!)</font>';

	disabled = '';
	# default values for the state and style of the form elements
	manual_selected = '';
	manual_style = 'block';
	...
	# check which mode is set in the set_date_global file, manual/use this pc time/ntp server time
	...
	if(form.getvalue("use_pc_time")):
		...
		if(set_date_cmd == True):
			unconf_ntp = ntp.unconfigure()
			#print """<script>location.href='main.py?page=date&valdate=success';</script>"""
			print"""<div id = 'id_trace' >"""
			print "Successfully set date!"
			print "</div>"
		else:
	...
	if(form.getvalue("username")):
		if(username == "Maintenance"):
			...
			#getpc=form.getvalue("pc")
			getserver=form.getvalue("server")
			#getdate=form.getvalue("getDate")
			print getmanual
			print getp
	...
	if(check_ha == True):
		print"""</span></a> Date/Time Settings of """+show_tn+"""
                <span style="float:right; margin:0 0px 0 0;"><a onclick="return onclick_loader();" class="resource_links" href="https://"""+other_node_ip+"""/fs4/py/iframe_date_set.py">"""+show_on+"""</a></span>
               		</div>"""
	else:
