...
if(form.getvalue('srp_ini_but')):
	...
	#print initiator_status
		if(initiator_status == True):
			...
			print "</div>"
			print "<script>location.href = 'iframe_srp_initiator.py?target="+target_name+"#tabs-1';</script>"
...
if(form.getvalue('choose_list')):
	...
	#print "<script>location.href = 'main.py?page=fc&act=del_ini_done';</script>"
	print "<script>location.href = 'iframe_srp_initiator.py?target="+target_name+"#tabs-2';</script>"
...
if (check_srp !=[]):
	...
	if (list_srp_target!= []):
		for initiator_target in list_srp_target:
			#print x
	...
	if (list_srp_target != []):
		for initiator_del_target in list_srp_target:
			#print x
	...
	<button class = 'buttonClass' type="submit" name = 'delete_initiator' value = 'Remove initiator' onclick ='return validate_srp_remove_ini();'>Remove</button>
