...
try:
	...
	if(form.getvalue("submit")):
		...
		get_folder_qo = form.getvalue("en_folder_quota")
		if(get_folder_qo == "on"):
			pquota = "yes"
		else:
			pquota = "no"

		create_nas = storage_op.lvcreate(volume_info,getdisk,getfull_size,pquota=pquota)	
		if(create_nas == True):
        		print""" <div id = 'id_trace_small' >"""
                	print " Nas Disk "+ getdisk +"Successfully created!"
			print "</div><br/>"
			logstatus = common_methods.sendtologs('Success', 'Successfuly Created the '+getdisk+' Disk', 'nas_configuration.py', str(create_nas));
		else:
			print""" <div id = 'id_trace_err_small' >"""

                	print "Error Occurred while Creating Nas configuration!"
                	print "</div><br/>"
			logstatus = common_methods.sendtologs('Error', 'Error Occurred while Creating the '+getdisk+' Disk', 'nas_configuration.py', str(create_nas));
	...
	<form name = 'nas_config' method = 'POST' action = ''>

<table width="90%" style="margin:20px 0 0 20px; text-align:center; border:#D1D1D1 1px solid;"">
<tr>
<td style=" background-color:#D1D1D1; padding:5px;">Create NAS Disk</td>
</tr>
</table>
...
<tr>
<td><input type="checkbox" name="en_folder_quota" id="en_folder_quota"/> Enable Folder Quota</td>
<td></td>
</tr>
...
<table id="adv_txt1"  width="90%" style="margin:0px 0 0 20px; padding:0 0 0 105px; border-left:#D1D1D1 1px solid; border-right:#D1D1D1 1px solid; font-size:10px; display:none;">
...
<tr>
<td>
<button class="buttonClass" style="float:right; margin:0 30px 20px 0; font-size:12px;" type="submit" name = 'submit'  id = 'submit' value = 'submit'  onclick = 'return validate_nas_configuration();'>Create</button>
</td>
</tr>
